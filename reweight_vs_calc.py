...
for idx in tqdm.tqdm(range(N_trials)):
    ...
    newpop.calculate_omega_gw(waveform_approximant=wave_approx, sampling_frequency=2048, Lambda=Lambda_new)
    new_omegas_reweight['Neff'].append( (np.sum(newpop.weights)**2)/(np.sum(newpop.weights**2)) )
...
for lamb in new_omegas_reweight['Lambdas']:
    ...
    new_omegas_calculate['omega_gw'].append()
    print(new_omegas_calculate)

new_omegas_reweight['freqs']=newpop2.frequency_array.tolist()
